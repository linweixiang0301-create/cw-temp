...
async function modelRoutingRegressionPayload(): Promise<Record<string, unknown>> {
  ...
  const checks = [
    ...
    },
    {
      ...
      status: 'ready',
      message: 'image 拆层失败回退单图层 manifest；vision 质检失败或未配置时返回 manual_review，不阻断 Photoshop / 飞书主链路。',
    },
    {
      code: 'usage_audit',
      status: history.length > 0 ? 'ready' : 'warning',
    ...
    if (req.method === 'POST' && url.pathname === '/api/psd-rebuild/jobs') {
      ...
      sendJson(res, 200, {
        ...(await createPsdRebuildJob({
          ...
          analysisRouteKey: body.analysisRouteKey === 'vision' ? 'vision' : 'image',
        })),
        jobs: listPsdRebuildJobs(),
      });
      return true;
